...
public class ShopKeeperDisplay : MonoBehaviour
{
    ...
    private void ClearSlots()
    {
        ...
        // Удаление всех дочерних объектов из панели _itemListContentPanel
        foreach (Transform child in _itemListContentPanel.transform)
        {
            Destroy(child.gameObject);
        }

        // Удаление всех дочерних объектов из панели _shoppingCartContentPanel
        foreach (Transform child in _shoppingCartContentPanel.transform)
        {
            Destroy(child.gameObject);
        }
    }
